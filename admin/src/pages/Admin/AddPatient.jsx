...
const AddPatient = () => {
  ...
  useEffect(() => {
    if (!telegramLink || !telegramTokenHash || !created?._id || !telegramConnectOpen) return undefined;
    let intervalId = setInterval(async () => {
      try {
        const { data: resp } = await axios.get(
          `${backendUrl}/api/admin/patients/${created._id}/telegram-check`,
        ...
        if (resp?.success && resp?.linked) {
          ...
          setCreated((prev) => prev ? ({
            ...prev,
            telegram: { isVerified: true, username: resp.patient?.telegram?.username }
          }) : null);
        }
      } catch (err) {
  ...
  }, [telegramLink, telegramTokenHash, created?._id, telegramConnectOpen, backendUrl, aToken]);
    ...
    try {
      ...
      try {
        const { data: linkData } = await axios.post(
          `${backendUrl}/api/admin/patients/${data.patient._id}/telegram-link`,
          {},
          { headers: { atoken: aToken } }
        );
        if (linkData?.success) {
        ...
        console.warn("Failed to automatically generate telegram link:", linkErr);
      }
