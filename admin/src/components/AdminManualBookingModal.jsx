...
const T = {
  uz: {
    ...
    selectDoctorPrompt: "Shifokor va boshlanish sanasini tanlang",
  ...
  ru: {
    ...
    selectDoctorPrompt: "Выберите врача и дату начала",
  ...
  en: {
    ...
    selectDoctorPrompt: "Select doctor and start date",
  ...
  tg: {
    ...
    notePlaceholder: "Масалан: бемор занг зад, дарди дандон, ташрифи аввал...",
    ...
    selectDoctorPrompt: "Духтур ва санаи оғозро интихоб кунед",
    freeSlotsCount: "{count} холӣ",
    cancel: "Рад кардан",
    saveBooking: "✓ Захираи вохӯрӣ",
    saving: "Захира шуда истодааст...",
    bookedSlotDetails: "Маълумоти вақti бандшуда",
    patientName: "Бемор",
    phone: "Тел",
    type: "Намуд",
...
const AdminManualBookingModal = ({
  ...
  const activeDoctors = useMemo(() => {
    return Array.isArray(dentists) ? dentists.filter(d => !d.isArchived) : [];
  }, [dentists]);

  // Locked doctor configuration
  const isDoctorLocked = Boolean(dentist);
  const lockedDoctorName = dentist?.name || "Mutaxassis";
  ...
  useEffect(() => {
    ...
    if (rescheduleAppointment) {
      ...
      const docId = rescheduleAppointment.dentistID?._id || rescheduleAppointment.dentistID || (dentist?._id || activeDoctors[0]?._id || "");
    ...
    const isLocked = Boolean(dentist);
    if (isLocked && !dentist?._id) {
      return; // wait until doctor is fully loaded
    }
    ...
    if (isLocked) {
      setSelectedDoctorId(dentist._id);
    } else if (activeDoctors.length > 0) {
      setSelectedDoctorId(activeDoctors[0]._id);
    } else {
      setSelectedDoctorId("");
    }

    hasInitializedRef.current = true;
  }, [open, dentist, activeDoctors, rescheduleAppointment]);
  ...
  useEffect(() => {
    if (!open || !selectedDoctorId || !calendarStartDate) {
      return;
    }
    ...
    const loadSchedule = async () => {
      ...
      try {
        const res = await getAdminCalendarAvailability(selectedDoctorId, calendarStartDate, 7);
        if (active) {
          if (res?.success) {
            setScheduleData(res);
          } else {
            toast.error(res?.message || "Taqvim yuklanmadi");
          }
        }
      } catch (err) {
  ...
  }, [open, selectedDoctorId, calendarStartDate, getAdminCalendarAvailability]);
  ...
  const handleCreatePatient = async () => {
    ...
    if (typeof createPatient !== "function") {
      toast.error("Yaratish funksiyasi topilmadi");
      return;
    }
  ...
  const handleCancelSlotAppointment = async (appId) => {
    if (!cancelAdminAppointment) return;
    const ok = window.confirm(t.cancelConfirmPrompt || "Uchrashuvni bekor qilishni xohlaysizmi?");
    if (!ok) return;
    try {
      const res = await cancelAdminAppointment(appId);
      if (res?.ok) {
        toast.success(t.cancelSuccess);
        setSelectedBusyApp(null);
        // Reload schedule
        const updated = await getAdminCalendarAvailability(selectedDoctorId, calendarStartDate, 7);
        if (updated?.success) setScheduleData(updated);
        onSubmit && onSubmit();
      }
    } catch (err) {
  ...
  const handleBook = async (e) => {
    ...
    if (!selectedDoctorId) {
      toast.error(lang === "tg" ? "Духтур бояд интихоб шавад" : lang === "ru" ? "Врач должен быть выбран" : lang === "en" ? "Doctor must be selected" : "Shifokor tanlanishi shart");
      return;
    }
    if (!selectedDate) {
    ...
    if (rescheduleAppointment) {
      try {
        const { data } = await axios.post(
          `${backendUrl}/api/admin/appointments/${rescheduleAppointment._id}/reschedule`,
          {
    ...
    const res = await createAdminManualAppointment({
    ...
    >
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-6xl max-h-[96vh] my-auto flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-start justify-between bg-gradient-to-r from-primary/5 to-transparent shrink-0">
          <div>
            <p className="text-[10px] font-semibold tracking-widest text-primary/60 uppercase mb-0.5">
              {isDoctorLocked ? t.roomDoctor : t.reception}
            </p>
        ...
        <form onSubmit={handleBook} className="flex-1 min-h-0 overflow-y-auto flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
          {/* Left panel: Patient & Doctor Selection */}
            ...
            <section className="space-y-2">
              ...
              {isDoctorLocked ? (
                ...
                <select
                  ...
                  className="w-full h-10 px-3.5 border border-slate-300 rounded-xl text-sm bg-white outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 transition"
                >
                  {activeDoctors.length === 0 ? (
                    <option value="">{t.noActiveDoctors}</option>
                  ) : (
                    activeDoctors.map((d) => (
                      <option key={d._id} value={d._id}>
                        {d.name} — {Array.isArray(d.speciality) ? d.speciality.join(", ") : d.speciality || ""}
                      </option>
                    ))
                  )}
                </select>
          ...
          <div className="flex-1 p-5 space-y-5 bg-slate-50/50 flex flex-col justify-between overflow-x-hidden">
            ...
            <div className="flex gap-2 pt-4 shrink-0 border-t border-slate-200/50 mt-4">
              ...
              <button
                type="submit"
                disabled={isSubmitting || !selectedPatient || !selectedDoctorId || !selectedTime}
