...
const ReviewsTrust = () => {
  return (
    <section aria-labelledby="reviews-heading" className="my-20 sm:my-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 sm:mb-14 gap-6">
        <div className="text-left max-w-2xl space-y-2">
          ...
          <h2 id="reviews-heading" className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-[#0F3040] leading-tight tracking-tight">
        ...
        <div className="bg-white rounded-3xl p-4 sm:p-5 border border-slate-200/90 shadow-card-clean flex items-center gap-4 shrink-0">
          <div className="w-12 h-12 rounded-2xl bg-amber-50 text-amber-500 flex items-center justify-center text-xl font-black border border-amber-200/80" aria-hidden="true">
            ★
          </div>
          <div className="text-left">
            <div className="flex items-center gap-1.5">
              <span className="text-2xl font-black text-[#0F3040]">4.9</span>
              <div className="flex text-amber-400 text-sm" aria-label="5 balldan 4.9 ball">★★★★★</div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
        {reviews.map((rev, i) => (
          <article
            key={i}
            className="bg-white rounded-[32px] p-6 sm:p-7 border border-slate-200/80 shadow-card-clean hover:shadow-card-hover hover:border-[#403D88]/40 transition-all duration-300 flex flex-col justify-between"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex text-amber-400 text-sm" aria-label={`${rev.rating} yulduzli baho`}>
                  {"★".repeat(rev.rating)}
                </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0F3040] to-[#321E48] text-white flex items-center justify-center font-black text-xs shrink-0 shadow-xs" aria-hidden="true">
                {rev.initials}
              </div>
              <cite className="text-left not-italic">
                <h3 className="text-xs sm:text-sm font-black text-[#0F3040] leading-snug">
                  {rev.name}
                </h3>
